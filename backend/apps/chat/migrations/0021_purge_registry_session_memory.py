"""Register chat.SessionMemory with the workspace purge contract.

chat.0020 added SessionMemory with a direct KnowledgeSpace FK.  The purge
oracle installed by knowledge.0015 hard-codes the direct-FK count (33) and
the registry row count (39), so workspace deletion would fail closed with
"workspace FK oracle changed".  This migration restores the contract:

1. registers chat.SessionMemory in spaces_workspacepurgedependency
   (migration-owned row, same pattern as knowledge.0015), and
2. reinstalls spaces_guard_workspace_delete() with the updated counts
   (FK 33 -> 34, registry 39 -> 40).
"""

from django.db import migrations

MIGRATION_OWNER = "chat.0021_purge_registry_session_memory"

OLD_REGISTRY_ROW_COUNT = 39
OLD_DIRECT_SPACE_FK_COUNT = 33
NEW_REGISTRY_ROW_COUNT = 40
NEW_DIRECT_SPACE_FK_COUNT = 34


def register_session_memory(apps, schema_editor):
    Registry = apps.get_model("spaces", "WorkspacePurgeDependency")
    # Same 7-tuple contract as chat.0018 rows: eligible content, purged
    # after chat.Message (80) and before chat.ChatTurn (90) — SessionMemory
    # cascades from ChatSession and carries no retained evidence.
    existing = Registry.objects.filter(
        model_label="chat.SessionMemory",
...
def unregister_session_memory(apps, schema_editor):
    Registry = apps.get_model("spaces", "WorkspacePurgeDependency")
...
def _workspace_delete_guard_sql(registry_row_count, direct_space_fk_count):
    # Verbatim copy of the guard installed by spaces.0016 / knowledge.0015,
    # with the two count oracles parameterized so this migration (and its
    # reverse) can reinstall the exact same contract at either revision.
    return f"""
...
class Migration(migrations.Migration):
    dependencies = [
        # 0020 created the SessionMemory FK the new oracle counts.
        ("chat", "0020_sessionmemory"),
        # knowledge.0015 installed the 33/39 guard being replaced — ordering
        # after it guarantees this migration's 34/40 guard is the final one.
        ("knowledge", "0015_purge_registry_review_models"),
    ]

    operations = [
        migrations.RunPython(
            register_session_memory,
            reverse_code=unregister_session_memory,
        ),
        migrations.RunPython(
